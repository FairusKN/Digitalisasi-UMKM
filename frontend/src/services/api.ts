...
const SUPERUSER_NAMES: string[] = rawEnvSuperusers
  ...
  .filter(Boolean);
const SUPERUSER_LIST = SUPERUSER_NAMES.length
  ? SUPERUSER_NAMES
  : DEFAULT_SUPERUSER_NAMES;
  ...
  async createProduct(
    ...
    try {
      if (imageFile) {
        ...
        try {
          ...
          if (err?.code === "ECONNABORTED" || !err?.response) {
            try {
              ...
              const listResp = await this.api.get("/manager/products", {
                timeout: 20000,
              });
              const productsData =
                listResp.data?.data?.data ||
                listResp.data?.data ||
                listResp.data ||
                [];
        ...
        const response = await this.api.post("/manager/products", cleanData, {
          timeout: 30000,
        });
        return response.data.data || response.data;
      }
      ...
      if (imageFile) {
        ...
        try {
          ...
          if (err?.code === "ECONNABORTED" || !err?.response) {
            try {
              await new Promise((res) => setTimeout(res, 800));
              const getResp = await this.api.get(`/manager/products/${id}`, {
                timeout: 20000,
              });
              ...
              const matches =
                ...
                (productData.price === undefined ||
                  Number(existing.price) === Number(productData.price)) &&
        ...
        if (productData.price !== undefined)
          cleanData.price = productData.price.toString();
        if (productData.category) cleanData.category = productData.category;
        if (productData.image !== undefined && productData.image !== "")
          cleanData.image = productData.image;
          ...
          if (err?.code === "ECONNABORTED" || !err?.response) {
            try {
              ...
              const matches =
                ...
                (cleanData.price === undefined ||
                  Number(existing.price) === Number(cleanData.price)) &&
                (!cleanData.category ||
                  existing.category === cleanData.category);
              if (matches) {
                return {
                  id: existing.id,
                  name: existing.name || "",
            ...
            await new Promise((res) => setTimeout(res, 1000));
            const retryResponse = await this.api.patch(
              `/manager/products/${id}`,
              cleanData,
              { timeout: 60000 }
            );
    ...
    try {
      try {
        ...
        if (err?.code === "ECONNABORTED" || !err?.response) {
          try {
            await new Promise((res) => setTimeout(res, 800));
            const listResp = await this.api.get("/manager/users", {
              timeout: 20000,
            });
            let usersData = [];
            if (
              listResp.data?.data?.data &&
              Array.isArray(listResp.data.data.data)
            ) {
